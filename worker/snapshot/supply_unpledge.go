...
func (w *Payee) handleUnpledgeEvent(ctx context.Context, tx *db.DB, output *core.Output, userID, followID string, body []byte) error {
	...
	if _, err := mtg.Scan(body, &ctokenAsset, &unpledgedAmount); err != nil {
		return w.handleRefundEvent(ctx, tx, output, userID, followID, core.ActionTypeUnpledge, core.ErrInvalidArgument, "")
	}
	...
	if isRecordNotFound {
		...
		return w.handleRefundEvent(ctx, tx, output, userID, followID, core.ActionTypeUnpledge, core.ErrMarketNotFound, "")
	}
	if e != nil {
	...
	if w.marketService.IsMarketClosed(ctx, market) {
		return w.handleRefundEvent(ctx, tx, output, userID, followID, core.ActionTypeUnpledge, core.ErrMarketClosed, "")
	}

	supply, isRecordNotFound, e := w.supplyStore.Find(ctx, userID, market.CTokenAssetID)
	if isRecordNotFound {
		log.Warningln("supply not found")
		return w.handleRefundEvent(ctx, tx, output, userID, followID, core.ActionTypeUnpledge, core.ErrSupplyNotFound, "")
	}
	if e != nil {
	...
	if unpledgedAmount.GreaterThan(supply.Collaterals) {
		...
		return w.handleRefundEvent(ctx, tx, output, userID, followID, core.ActionTypeUnpledge, core.ErrInsufficientCollaterals, "")
	}
	...
	if unpledgedTokenLiquidity.GreaterThan(liquidity) {
		...
		return w.handleRefundEvent(ctx, tx, output, userID, followID, core.ActionTypeUnpledge, core.ErrInsufficientLiquidity, "")
	}
	...
	return w.transferOut(ctx, tx, userID, followID, output.TraceID, market.CTokenAssetID, unpledgedAmount, &transferAction)
}
